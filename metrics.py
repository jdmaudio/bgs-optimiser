...
class ConfusionMatrix:
    def __init__(self, truth, detections, mask):
    ...
    def compute(self):
        positive_count = np.count_nonzero(np.bitwise_and(self.detections, self.mask))
        negative_count = np.count_nonzero(np.ones(self.truth.shape[0]) - np.bitwise_and(self.detections, self.mask))

        temp = np.bitwise_and(np.bitwise_and(self.truth, self.mask), self.detections)
        self.TP = np.count_nonzero(temp)
        self.FP = positive_count - self.TP

        temp = np.bitwise_not(np.bitwise_and(self.detections, self.mask))
